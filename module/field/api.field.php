...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function fieldGet($opt=array()){
		...
		if($opt['debug']) $this->helper->pre("OPT", $opt);
		...
		# On demande des FIELD directement
		#
		...
		$query	= "SELECT * FROM k_field ".$where.$order;
		$field	= $this->mysql->$dbMode($query);

		if($opt['debug']) $this->pre("QUERY", $this->mysql->db_query(), "ERROR", $this->mysql->db_error(), $field);
...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function fieldSet($id_field, $def){
		...
		$query = (intval($id_field) > 0)
			? $this->mysql->update($def)." WHERE id_field=".$id_field
			: $this->mysql->insert($def);

		$this->mysql->query($query);
		#$this->pre($this->db_query, $this->db_error);

		if($this->mysql->db_error() != NULL) return false;
		$this->id_field = ($id_field > 0) ? $id_field : $this->mysql->db_insert_id();

		$this->fieldAffectType($this->id_field);
...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function fieldRemove($id_field){
		...
		if(intval($id_field) == 0) return false;

		// Remove FIELD
		$this->mysql->query("DELETE FROM k_field		WHERE id_field=".$id_field);
		$this->mysql->query("DELETE FROM k_fieldchoice	WHERE id_field=".$id_field);
...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function fieldChoiceGet($opt){
		...
		$field = $this->mysql->$dbMode("SELECT * FROM k_fieldchoice ". $where . $order);
		
		if($opt['debug']) $this->pre($this->mysql->db_query(), $this->mysql->db_error(), $field);
	...
	public function fieldChoiceSet($id_field, $es){
		...
		foreach($es as $id_fieldchoice => $e){
			if($id_fieldchoice == 'remove'){
				foreach($e as $r){
					$this->mysql->query("DELETE FROM k_fieldchoice WHERE id_fieldchoice = ".$r);
				}
			}else
			if($id_fieldchoice == 'new'){
	
				$last = $this->mysql->one("SELECT MAX(choiceOrder) AS d FROM k_fieldchoice WHERE id_field=".$id_field);
				$last = $last['d'] + 1;
				...
				if($previous['choiceValue'] == $e){
					...
					$this->fieldAffectValue($id_field, $previous['choiceValue'], $e);
				}
	
				$this->mysql->query($this->mysql->update($def)." WHERE id_fieldchoice=".$id_fieldchoice);
				#$this->pre($this->db_query, $this->db_error);
				$pos++;
			}
		}
	...
	public function fieldSaveValue($id_field, $value, $opt=array()){
		...
		if($field_['fieldType'] == 'keyword'){
			...
			if(intval($opt['id_content']) > 0 && $opt['language'] != ''){
				$this->mysql->query("DELETE FROM k_contentkeyword WHERE id_field=".$id_field." AND id_content=".$opt['id_content']." AND language='".$opt['language']."'");
				#$this->pre($this->db_query, $this->db_error);
	
				if(sizeof($save) > 0){
					$this->mysql->query("INSERT IGNORE INTO k_contentkeyword (id_field, id_content, language, keyword) VALUES ".implode(",\n", $save));
					#$this->pre($this->db_query, $this->db_error);
				}
			}
		...
		if(is_array($value)){
			...
			if($field_['fieldType'] == 'user'){
				...
				if($opt['id_content'] != NULL){
					$content_ = $this->mysql->one("SELECT id_type FROM k_content WHERE id_content=".$opt['id_content']);
...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function fieldTypeGet(){
		...
		$def = array(
			...
			'texte-line'	=> array('name' => 'Texte sur une  ligne',          'view' => 'texte',		 'type' => 'VARCHAR(255)'),
			...
			'multichoice'	=> array('name' => 'Plusieurs choix',		        'view' => 'choice', 	 'type' => 'MEDIUMTEXT'),
...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function fieldAffectGet($map, $id=0){
		$def = $this->mysql->one("SELECT fields FROM k_fieldaffect WHERE map='".$map."' AND id=".intval($id));
...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function fieldAffectSet($map, $fields, $id=0){
		...
		$this->mysql->query("
			INSERT INTO k_fieldaffect (map,id,fields)
			VALUES ('".$map."', ".intval($id).", '".$json."') ON DUPLICATE KEY UPDATE fields='".$json."'"
		);
		#$this->pre($this->db_query, $this->db_error);
	}
...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function fieldAffectPush($map, $field, $id=0){
		...
		// Get
		$mymap		= $this->mysql->one("SELECT * FROM k_fieldaffect WHERE map='".$map."' AND id='".$id."'");
...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function fieldAffectPop($map, $field, $id=0){
		...
		// Get
		$map = $this->mysql->one("SELECT * FROM k_fieldaffect WHERE map='".$map."' AND id='".$id."'");
...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function fieldAffectNew($map, $id_field, $id=0){
		...
		$myFields = $this->mysql->multi("SHOW COLUMNS FROM " . $table);
		$found    = false;
		...
		if(!$found){
			$this->mysql->query("ALTER TABLE ".$table." ADD field".$id_field." VARCHAR(255) NOT NULL");
			#$this->pre($this->db_query, $this->db_error);
		}
...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function fieldAffectRemove($id_field, $map='ALL', $id=0){
		...
		#
		if($map == 'ALL'){
			$def = $this->mysql->multi("SELECT * FROM k_fieldaffect WHERE map != 'usergroup'");
			...
			$used[] = $this->mysql->one("SELECT * FROM k_fieldaffect WHERE map = '".$map."' AND id=".intval($id));
		}
		...
		foreach($used as $e){
			...
			$this->mysql->query("ALTER TABLE ".$table." DROP field".$id_field);
			#$this->pre($this->db_query, $this->db_error);
...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function fieldAffectType($id_field, $table=NULL){
		...
		#
		foreach($use as $e){
			$fields = $this->mysql->multi("SHOW COLUMNS FROM `".$e."`");
			foreach($fields as $f){
				if($f['Field'] == 'field'.$id_field){
					if(strtolower($f['Type']) != strtolower($type)){
						$this->mysql->query("ALTER TABLE `".$e."` CHANGE `field".$id_field."` `field".$id_field."` ".$type." NOT NULL");
						#$this->pre($this->db_query, $this->db_error);
					}
				}
			}
