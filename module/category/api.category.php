...
namespace Kodeine;
	...
	class category extends appModule{

//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function categoryGet($opt=array()){

		if(BENCHME) $this->bench->marker($bmStep='categoryGet() @='.json_encode($opt));
		...
		if($opt['language'] == '??' && $opt['id_category'] != ''){
			$tmp = $this->mysql->multi("SELECT language FROM k_categorydata WHERE id_category=".$opt['id_category']);
		...
		if($opt['thread']){
			...
			$category	= $this->mysql->multi("SELECT * FROM k_category".$inner."\n".$where." ORDER by pos_category");
...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function categorySet($id_category, $def, $lan){
		...
		if($id_category == ''){
			...
			$max = $this->mysql->one("SELECT MAX(pos_category) as last FROM k_category WHERE mid_category=".(($mid >= 0) ? $mid : 0));
		...
		$this->mysql->query($this->dbUpdate($def)." WHERE id_category=".$id_category);
		#$this->pre($this->db_query, $this->db_error);


		# Gerer les differentes LANGUES
		foreach($lan as $iso => $e){
			$exists = $this->mysql->one("SELECT 1 FROM k_categorydata WHERE language='".$iso."' AND id_category=".$id_category);
			if(!$exists[1]) $this->mysql->query("INSERT INTO k_categorydata (id_category, language) VALUES (".$id_category.", '".$iso."')");
		    ...
		    @$this->mysql->query($this->dbUpdate($me)." WHERE id_category=".$id_category." AND language='".$iso."'");
			#$this->pre($this->db_query, $this->db_error);
		}
...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function categoryRemove($id_category){
		...
		#
		foreach($this->mysql->multi("SELECT * FROM k_userprofile") as $p){
		...
		if($me['categoryChildren'] != NULL){
			$this->mysql->query("DELETE FROM k_category 			WHERE id_category IN(".$me['categoryChildren'].")");
			$this->mysql->query("DELETE FROM k_categorydata 		WHERE id_category IN(".$me['categoryChildren'].")");
			$this->mysql->query("DELETE FROM k_contentcategory 	WHERE id_category IN(".$me['categoryChildren'].")");
		}

		$this->mysql->query("DELETE FROM k_category			WHERE id_category = ".$me['id_category']);
		$this->mysql->query("DELETE FROM k_categorydata		WHERE id_category = ".$me['id_category']);
		$this->mysql->query("DELETE FROM k_contentcategory	WHERE id_category = ".$me['id_category']);
...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function categoryFamily(){
		...
		foreach($this->tempCat as $id_category => $tree){
			$this->mysql->query("UPDATE k_category  SET categoryParent='".substr($tree, 0, -1)."' WHERE id_category=".$id_category);
		}
...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function categoryCache(){
		...
		$items = $this->mysql->multi("SELECT * FROM k_categorydata");
...
//-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
	public function categoryUpdatePos($category){
		...
		foreach($category as $pos => $e){
			$this->mysql->query("UPDATE k_category SET pos_category=".$pos." WHERE id_category=".$e['id']);
