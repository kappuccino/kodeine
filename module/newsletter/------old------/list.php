...
<div style="float:left; width:35%; margin-right:20px;">
	...
	<form action="list" method="get" style="margin-top:20px;">
		...
		<div class="clearfix">
			<input type="text" name="speed" style="float:left; width:200px;" />
			<input type="submit" value="Abonner / Désabonner ce membre" style="float:right;" />
		</div>
	</form>
		...
		if(isset($_REQUEST['user'])){
			...
			<script>
				...
				function kill(){
					var chbx = $('.cbk');
					var count = 0;
					chbx.each(function(me){
						if(me.checked) count++;
					});

					if(count == 0) return false;
					if(confirm("Voulez vous supprimer tous les mails selectionné ?")){
						$('#mails').submit();
					}
				}
