...
class chapter extends appModule{
	...
	function get($opt=array()){
		...
		if($opt['thread']){
			...
			$chapter = $this->multi("SELECT * FROM k_chapter ".$inner . $where." ORDER by pos_chapter");
		...
		if($dbMode == 'one') unset($limit, $order);
		...
		if(sizeof($chapter) > 0){
			if($dbMode == 'one') $chapter = array($chapter);
	...
	private function chapterNewAffect($id_chapter){
		...
		$asso = $this->multi("SELECT id_content FROM k_contentchapter WHERE id_chapter=".$me['mid_chapter']);
		foreach($asso as $e){
	...
	public function chapterProfile($id_chapter){
		...
		$profile		= $this->one("SELECT * FROM k_userprofile WHERE id_profile=".$id_profile);
