...
$success	= false;
#$folder 	= KROOT.urldecode($_REQUEST['folder']);
$folder 	= KROOT.rawurldecode($_REQUEST['folder']);
	...
	case 'folder' :
		if(file_exists($folder)){
			...
			if($_REQUEST['id_album'] > 0){
				$app->dbQuery("UPDATE k_contentalbum SET contentAlbumSyncFolder='".urldecode($_REQUEST['folder'])."' WHERE id_content=".$_REQUEST['id_album']);
			}

			// Balancer la sauce sur les dossier
			fld($folder, $_REQUEST['id_album'], $_REQUEST['id_type'], $_REQUEST['into']);
