<?php
	...
	if($_REQUEST['end'] == 'true'){
		$app->dbQuery("UPDATE k_newsletter SET newsletterSendDate=NOW() WHERE id_newsletter=".$id_newsletter);
	...
	if($_REQUEST['send'] == 'true'){
		...
		$mails	= array_chunk($mails, $_POST['chunk']);
		$mails	= $mails[$_POST['current']];
